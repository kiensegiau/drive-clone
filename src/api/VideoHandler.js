...
class VideoHandler {
    ...
    this.drive = google.drive({
      ...
      authHandler: async () => {
        try {
          ...
          const { tokens: newTokens } = await this.oAuth2Client.refreshToken(
  ...
  async _processVideo(fileId, fileName, targetFolderId, depth, profileId) {
    ...
    try {
      ...
      videoUrl = await this.retryOperation(async () => {
          ...
          try {
            if (
              ...
              try {
                ...
                // Nu ã tìm được đủ URL, chọn URL chất lượng cao nhất
        ...
        page.on("request", (request) => {
          try {
            ...
            console.log(
              `${indent}⚠️ Không thể continue request:`,
              error.message
            );
          }
        });
      ...
      const downloadPromise = this.downloadVideoWithChunks(
        videoUrl,
        outputPath
      );
      ...
      downloadPromise.then(async () => {
        try {
          ...
          console.error(
            `${indent}❌ Lỗi xử lý video ${fileName}:`,
            error.message
          );
        }
      });
        ...
        writer.on("finish", async () => {
          ...
          if (stats.size === 0) {
            if (retryCount < MAX_DOWNLOAD_RETRIES) {
              console.log(
                `\n⚠️ File ti xuống rỗng, đang thử lại lần ${retryCount + 1}...`
              );
              writer.close();
  ...
  async uploadFile(filePath, fileName, targetFolderId, mimeType) {
    ...
    for (let attempt = 0; attempt < MAX_RETRIES; attempt++) {
      try {
        ...
        // Chỉ dùng các metadata cần thiết
        const fileMetadata = {
          name: fileName,
          parents: [targetFolderId]
        };

        // Upload file với cấu hình đơn giản
        const media = {
          mimeType: mimeType,
